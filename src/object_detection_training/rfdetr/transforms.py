...
# See LICENSE file in original repository for license terms.
#
...
class Pad(object):
    def __init__(
        ...
        Args:
            size: image target size, if None, pad to multiple of size_divisor
            size_divisor: size divisor, default 32
            pad_mode: pad mode, currently only supports four modes [-1, 0, 1, 2]
            offsets: [offset_x, offset_y], specify offset while padding
            fill_value: rgb value of pad area
        """
        if not isinstance(size, (int, Sequence)):
            raise TypeError(
                f"Type of target_size is invalid. Must be List, now is {type(size)}"
            )
